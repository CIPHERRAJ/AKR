...
import { Calculator, Send, TrendingUp, Info, Sparkles, Lock, ArrowRight, User, Gem, Plus, Trash2 } from 'lucide-react';
...
function App() {
  ...
  const [oldGoldItems, setOldGoldItems] = useState<OldGoldItem[]>([
    { id: Date.now(), weight: '', touch: '', rate: '' }
  ]);

  const addOldGoldItem = () => {
    setOldGoldItems(prev => [...prev, { id: Date.now(), weight: '', touch: '', rate: '' }]);
  };

  const removeOldGoldItem = (id: number) => {
    if (oldGoldItems.length > 1) {
      setOldGoldItems(prev => prev.filter(item => item.id !== id));
    }
  };

  const updateOldGoldItem = (id: number, field: keyof OldGoldItem, value: string) => {
  ...
  const calculateTotal = () => {
    ...
    setTimeout(() => {
      ...
      let totalProcessedOldWeight = 0;
      let totalExchangeValue = 0;

      oldGoldItems.forEach(item => {
        const oldW = Math.max(0, safeParse(item.weight));
        const oldT = safeParse(item.touch);
        const touchFactor = oldT > 0 ? oldT / 100 : 1;
        const oldR = Math.max(0, safeParse(item.rate));

        const processedWeight = oldW * touchFactor;
        const itemExchangeValue = processedWeight * oldR;

        totalProcessedOldWeight += processedWeight;
        totalExchangeValue += itemExchangeValue;
      });

      const netPayable = total - totalExchangeValue;
      ...
      setResult({
        ...
        processedOldWeight: totalProcessedOldWeight
      });
...
${oldGoldItems.map((item, idx) => {
  const w = parseFloat(item.weight) || 0;
  if (w === 0) return '';
  const t = parseFloat(item.touch) || 0;
  const net = w * (t/100);
  return `#${idx+1}: ${w}g @ ${t}% = ${net.toFixed(3)}g (₹${item.rate}/g)`;
}).filter(Boolean).join('\n')}
Total Net Weight: ${result.processedOldWeight.toFixed(3)}g
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md glass-panel rounded-3xl p-8 border border-neutral-800 animate-slide-up">
          <div className="flex flex-col items-center mb-8">
            <div className="bg-gradient-to-br from-yellow-400 to-yellow-600 p-4 rounded-2xl shadow-xl shadow-yellow-500/20 mb-4">
        ...
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="bg-gradient-to-br from-yellow-400 to-yellow-600 p-2 rounded-xl shadow-lg shadow-yellow-500/20">
              <Gem className="w-5 h-5 text-black fill-black/10" />
            </div>
            <div>
          ...
          <div className="bg-neutral-950/50 rounded-[20px] p-5 space-y-6">
            ...
            <div className="bg-neutral-900/30 rounded-xl p-4 border border-neutral-800/50 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={addOldGoldItem}
                  className="bg-neutral-800 hover:bg-neutral-700 text-neutral-400 hover:text-white p-1.5 rounded-lg transition-colors"
                  title="Add Item"
                >
                  ...
                  <div key={item.id} className="relative group">
                    <div className="grid grid-cols-12 gap-2">
                      <div className="col-span-4 relative">
                        {index === 0 && <label className="absolute -top-2 left-2 bg-neutral-900 px-1 text-[10px] text-neutral-500 uppercase">Old Wt</label>}
                        <input
                          type="number"
                          inputMode="decimal"
                          value={item.weight}
                          onChange={(e) => updateOldGoldItem(item.id, 'weight', e.target.value)}
                          onWheel={(e) => (e.target as HTMLInputElement).blur()}
                          placeholder="Wt"
                          className="w-full bg-neutral-900/50 border border-neutral-800 rounded-xl px-2 py-3 text-sm font-medium outline-none focus:border-red-500/30 transition-colors text-right"
                        />
                      </div>
                      <div className="col-span-3 relative">
                        {index === 0 && <label className="absolute -top-2 left-2 bg-neutral-900 px-1 text-[10px] text-neutral-500 uppercase">Touch</label>}
                        <input
                          type="number"
                          inputMode="decimal"
                          value={item.touch}
                          onChange={(e) => updateOldGoldItem(item.id, 'touch', e.target.value)}
                          onWheel={(e) => (e.target as HTMLInputElement).blur()}
                          placeholder="%"
                          className="w-full bg-neutral-900/50 border border-neutral-800 rounded-xl px-2 py-3 text-sm font-medium outline-none focus:border-red-500/30 transition-colors text-right"
                        />
                      ...
                      <div className="col-span-4 relative">
                        {index === 0 && <label className="absolute -top-2 left-2 bg-neutral-900 px-1 text-[10px] text-neutral-500 uppercase">Rate</label>}
                        <input
                          type="number"
                          inputMode="decimal"
                          value={item.rate}
                          onChange={(e) => updateOldGoldItem(item.id, 'rate', e.target.value)}
                          onWheel={(e) => (e.target as HTMLInputElement).blur()}
                          placeholder="Rate"
                          className="w-full bg-neutral-900/50 border border-neutral-800 rounded-xl px-2 py-3 text-sm font-medium outline-none focus:border-red-500/30 transition-colors text-right"
                        />
              ...
              {oldGoldItems.some(i => i.weight && i.touch) && (
                <div className="flex justify-between items-center bg-neutral-950/50 px-3 py-2 rounded-lg border border-neutral-800/50">
                  <span className="text-[10px] text-neutral-500 uppercase font-bold">Total Net Exchange Weight</span>
                  <span className="text-sm font-bold text-red-400 font-mono">
                    {oldGoldItems.reduce((acc, item) => {
                      const w = parseFloat(item.weight) || 0;
                      const t = parseFloat(item.touch) || 0;
                      return acc + (w * (t / 100));
                    }, 0).toFixed(3)}g
